...
const KycRequested = () => {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const res = await apiRequest('/kyc/pending');
  ...
  const handleReview = async (id, status, remarks = "") => {
    try {
      setActionLoading(id);
      await apiRequest('/kyc/review', 'POST', { requestId: id, status, remarks });
      // Refresh list
      fetchRequests();
    } catch (error) {
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <tr key={row.id} className="hover:bg-cyan-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-slate-800">{row.user.name}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-slate-700">{row.user.mobile}</div>
                    <div className="text-xs font-medium text-slate-500 mt-0.5">{row.user.email}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex flex-col gap-1 text-[10px] font-bold">
                      <a href={row.aadharFront} target="_blank" rel="noreferrer" className="text-cyan-600 hover:underline flex items-center gap-1"><Eye size={12} /> Aadhar Front</a>
                      <a href={row.aadharBack} target="_blank" rel="noreferrer" className="text-cyan-600 hover:underline flex items-center gap-1"><Eye size={12} /> Aadhar Back</a>
                      <a href={row.panImage} target="_blank" rel="noreferrer" className="text-cyan-600 hover:underline flex items-center gap-1"><Eye size={12} /> PAN Card</a>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-xs">
                    <div className="font-bold text-slate-700">{row.user.bankName}</div>
                    <div className="text-slate-500">{row.user.accountNo}</div>
                    <div className="text-slate-400">IFSC: {row.user.ifscCode}</div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        title="Approve"
                      >
                        {actionLoading === row.id ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />}
                      </button>
                      <button
                        onClick={() => handleReview(row.id, 'REJECTED')}
                        disabled={actionLoading === row.id}
                        className="p-1.5 bg-red-50 text-red-600 border border-red-100 rounded-lg hover:bg-red-100 transition-colors disabled:opacity-50"
                        title="Reject"
                      >
                        {actionLoading === row.id ? <Loader2 size={16} className="animate-spin" /> : <CloseIcon size={16} />}
                      </button>
                    </div>
                  </td>
                </tr>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <p className="text-sm font-medium text-slate-500">
            Showing <span className="font-bold text-slate-700">1</span> to <span className="font-bold text-slate-700">{requests.length}</span> entries
          </p>
